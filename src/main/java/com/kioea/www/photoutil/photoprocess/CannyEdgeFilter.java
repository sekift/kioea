...
public class CannyEdgeFilter extends AbstractBufferedImageOp {
	...
	public BufferedImage filter(BufferedImage src, BufferedImage dest) {
		...
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				...
				float xg = (getPixel(outPixels, width, height, col, row + 1)
						...
						+ getPixel(outPixels, width, height, col + 1, row + 1) - getPixel(outPixels, width, height,
						col + 1, row)) / 2.0f;
				float yg = (getPixel(outPixels, width, height, col, row)
						- getPixel(outPixels, width, height, col + 1, row)
						+ getPixel(outPixels, width, height, col, row + 1) - getPixel(outPixels, width, height,
						col + 1, row + 1)) / 2.0f;
				// 计算振幅与角度
		...
		for (int i = 0; i < inPixels.length; i++) {
			int gray = ProcessUtil.clamp((int) data[i]);
	...
	public static void main(String args[]) {
		...
		try {
			// 读入图像
			biRead = ImageIO.read(new File("D:/uxnu.jpg"));
			// 对象
			CannyEdgeFilter sf = new CannyEdgeFilter();
			// 输出图像(处理后的图像，格式，路径)
			ImageIO.write(sf.filter(biRead, null), "JPEG", new File("D:/uxnucannyedge.jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
