...
public class RandomForMobile {
	...
	public static void main(String[] args) {
		int n = 10000;// 容量总数，位数
		int m = 10000;// 生成总数
		int k = 31;// 对userId求m余后最接近的一个素数，保证不同id不同起始
		int t = 29;// 根据已经生产的个数来做运算，当i为0个重新获取[此数必须与n互素，而且一个生产周期必须相同]
		int exsit = 0; // 已经生产数的个数
		int j = exsit + 10000; //  批量，生产第i+1到j的个数
		// RandomForMobile.middleAlg(n, m, k);
		RandomForMobile.randomFromCicleAlg(n, m, k, t, exsit, j);
	}
}
